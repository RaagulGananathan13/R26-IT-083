...
/**
 * Say which generator wrote the text, and when the gate refused one.
 *
 * A reader cannot check a report they think came from somewhere else. The
 * neural model is served only when its output asserts exactly the findings the
 * conformal layer ruled in; when it does not, the deterministic template is
 * returned instead, and that substitution is the safety mechanism working
 * rather than something to hide.
 */
function reportDescription(result: Envelope): string {
  const source = (result.raw as Record<string, unknown> | undefined)?.reportSource;
  if (source === "neural") {
    return "Generated by the fine-tuned report model, and released only because " +
      "its findings matched the conformal rule-in set exactly.";
  }
  if (typeof source === "string" && source.includes("verification failed")) {
    return "The generated report did not preserve the findings, so the " +
      "deterministic template was served instead.";
  }
  return "Template-grounded and passed through an automated verification gate before release.";
}
    ...
    >
      <StudyGrid wide={Boolean(result && !pending)}>
        <div className="space-y-4">
          {result && !pending ? (
            <LoadedStudy
              name={dat?.name.replace(/\.dat$/i, "") ?? "WFDB record"}
        ...
        <div className="space-y-4">
          ...
          {result && !pending && (
            <div className="space-y-4">
              ...
              {/* The strip and its attention curve beside the report they
                  produced, so a claim in the text can be checked against the
                  part of the trace the model actually attended to. */}
              <AnalysisSplit
                className="rise-in-1"
                left={
                  <div className="space-y-4">
                    ...
                    />
                    <TemporalAttention
                      curve={(result.explanation.cam as number[] | undefined) ?? []}
                      durationSeconds={(result.explanation.camSeconds as number | undefined) ?? 0}
                      peaks={result.explanation.peaksSeconds as number[] | undefined}
                      note={result.explanation.camNote as string | undefined}
                    />
                  </div>
                }
                right={
