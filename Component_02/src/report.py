...
def _quality_line(q: QualityReport) -> str:
    ...
    if q.out_of_scope:
        parts.append("RHYTHM OUTSIDE THE MODEL'S FIVE CLASSES — "
                     "arrhythmia not assessed.")
...
def build_report(probs: Dict[str, float] | List[float],
    ...
    guarantees: List[str] = []
    if quality.out_of_scope:
        guarantees.append(
            "STATISTICAL GUARANTEES WITHHELD: the rhythm appears to lie outside "
            "the five diagnostic classes this model was trained on (irregularly "
            "irregular R-R response, characteristic of atrial fibrillation or "
            "flutter). No conformal bound covers a class that is not in the "
            "label space. The findings below concern ONLY the five superclasses "
            "and must not be read as excluding an arrhythmia.")
    ...
    _suppress = quality.electrode_suspect or quality.out_of_scope
    for cls in (ruled_out if not _suppress else []):
        t = thresholds.get(cls, {})
    ...
    if quality.out_of_scope:
        limitations += [f"Rhythm scope: {d}" for d in quality.scope_detail]
