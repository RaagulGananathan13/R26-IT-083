...
def _hallucinated_terms(text: str) -> List[str]:
    """Forbidden diagnoses that are ASSERTED (not merely named as undetectable)."""
    low = text.lower()
    hits = []
    for term in FORBIDDEN_TERMS:
        for m in re.finditer(re.escape(term), low):
            ...
            # The LIMITATIONS block legitimately names them as undetectable.
            if "not detected" in clause or "recognises only" in clause:
                continue
            hits.append(term)
            break
    return hits
