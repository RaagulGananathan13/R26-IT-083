...
16. [Ethics and data use](#16-ethics-and-data-use)
...
│   │   ├── ecg_waveform.py       #   ST-segment measurement, ESC/AHA criteria
...
├── artifacts/{data,models,reports,figures}
...
Point `paths.raw_dir` in `configs/config.yaml` at the folder holding the extracted MIMIC parquet files (`master_data`, `lab_values`, `medrecon`, `ecg_records`, `ecg_measurements`, `ecg_numeric`, `charlson`).
